...
class QwenVLDataCollator:
    ...
    def __call__(self, batch):
        ...
        for item in batch:
            ...
            slot_texts.append(json.dumps(scene_slot, ensure_ascii=False))
        ...
        full_inputs["slot_input_ids"] = slot_inputs["input_ids"]
        full_inputs["slot_attention_mask"] = slot_inputs["attention_mask"]
        full_inputs["question_input_ids"] = question_inputs["input_ids"]
        full_inputs["question_attention_mask"] = question_inputs["attention_mask"]
...
def build_model_and_processor(model_path, use_bf16=True):
    ...
    if hasattr(processor, "tokenizer") and processor.tokenizer is not None:
        processor.tokenizer.add_special_tokens({"additional_special_tokens": [SLOT_TOKEN]})
...
def main():
    ...
    if slot_token_id is None or slot_token_id < 0:
        raise RuntimeError("Slot token id not found; check tokenizer special tokens.")
