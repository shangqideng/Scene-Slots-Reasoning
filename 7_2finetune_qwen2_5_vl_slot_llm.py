...
class JsonlIndexDataset(Dataset):
    def __init__(self, path, max_samples=None, skip_missing_image=False):
        self.path = path
        self.offsets = []
...
class QwenVLDataCollator:
    def __init__(self, processor, image_size=None, max_text_tokens=None):
...
def load_fsdp_config(path, model=None):
    if not path:
        return None
    ...
    if model is None or not isinstance(cfg, dict):
        return cfg
    layer_names = cfg.get("fsdp_transformer_layer_cls_to_wrap")
    if not isinstance(layer_names, list):
        return cfg
    model_name = model.__class__.__name__
    # For Qwen2 LLMs, use the decoder layer class name expected by peft FSDP helper.
    if "Qwen2" in model_name and "Qwen2_5_VL" not in model_name:
        if "Qwen2DecoderLayer" not in layer_names:
            cfg = dict(cfg)
            cfg["fsdp_transformer_layer_cls_to_wrap"] = ["Qwen2DecoderLayer"]
    return cfg
...
def build_arg_parser():
    ...
    parser.add_argument(
        "--model-path",
        default="/data3/lxp/Models/Qwen2.5-3B-Instruct",
    ...
    parser.add_argument(
        "--fsdp-config",
        default="./fsdp_config_qwen2.json",
...
def main():
    ...
    if fsdp and str(fsdp).lower() in ("none", "false", "0"):
        fsdp = None
    fsdp_config = load_fsdp_config(args.fsdp_config, model=model) if fsdp else None
