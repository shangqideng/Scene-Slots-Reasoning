...
## Local QA jsonl segmentation (SAM2)

This repo includes a helper script `5sam2_segment.py` to:
- Parse object names and prompts from QA jsonl questions (color point/bbox).
- Scale point/bbox coordinates from the 0-1000 space to image pixels.
- Run SAM2 image segmentation per object and keep the best mask.
- Save per-sample segmentation data into a new jsonl (`sam2_seg` field).
- Visualize a single sample by id.
...
python 5sam2_segment.py \
  --jsonl /data/dsq/ScanNet/qa_jsonl/train.jsonl \
  --model /data3/lxp/Models/sam2.1-hiera-large \
  --vis-id scene0001_00_0 \
  --vis-out ./scene0001_00_0_sam2.png
```