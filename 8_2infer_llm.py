...
def build_prompt(tokenizer, question: str, scene_slot=None, use_scene_slot: bool = False) -> str:
    if use_scene_slot:
    ...
    with open(out_jsonl, "w", encoding="utf-8") as f_out:
        ...
        for i, sample in enumerate(data):
            ...
            if use_scene_slot and scene_slot is None:
                ...
                continue
            prompt = build_prompt(tokenizer, question, scene_slot, use_scene_slot)
            ...
            if len(batch_prompts) >= batch_size:
                _run_hf_batch(
                    model, tokenizer, batch_prompts, batch_meta, f_out, max_tokens
                )
...
def _run_hf_batch(model, tokenizer, prompts, metas, f_out, max_tokens):
